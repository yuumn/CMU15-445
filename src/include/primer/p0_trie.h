...
class TrieNode {
 public:
  ...
  int ChildrenSize() { return children_.size(); }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < len; i++) {
      if ((*p)->HasChild(key[i])) {
        p = (*p)->GetChildNode(key[i]);
      } else {
        p = (*p)->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      }
    }
    if ((*p)->IsEndNode()) {
      latch_.WUnlock();
    ...
    auto newnode = new TrieNodeWithValue<T>(std::move(**p), value);
    p->reset(newnode);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    std::vector<std::unique_ptr<TrieNode> *> ve;
    for (int i = 0; i < len; i++) {
      fa = p;
      ve.push_back(fa);
      if ((*p)->HasChild(key[i])) {
    ...
    while (!ve.empty()) {
      ...
      (*fa)->RemoveChildNode((*p)->GetKeyChar());
      p = fa;
    }
    latch_.WUnlock();
    return true;
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto *rt = dynamic_cast<TrieNodeWithValue<T> *>(p->get());
    if (rt == nullptr) {
      *success = false;
      latch_.RUnlock();
      return {};
